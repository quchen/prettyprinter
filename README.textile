...
The library provides the obvious API, which just allows coloration of any @Doc@ value. It looks something like this:
...
Due to a technical limitation with the ansi-terminal package, currently colors will only show up if you output using @putDoc@. It certainly won't work if you output to a string, and even won't even work if you try and output to a handle, regardless of whether that handle is for an output stream on an ANSI enabled terminal or not.
